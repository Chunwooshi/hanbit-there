...
$.ajax({
    url: 'http://localhost:82/api/there/guam',
...
function initThere(model) {
    $('.ht-sect-there').css('background-image',
        'url(' + model.background + ')');
    $('.ht-there-n').html(model.name);
    $('.ht-there-s').html(model.summary);
    ...
    $('.ht-tra-list').html(areaTrafficHtml);
    ...
    loadGoogleMapsApi().then(function(googleMaps) {
        ...
        areaMap = new googleMaps.Map($('#area-map')[0], {
